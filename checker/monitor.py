...
assert all([SLACK_CHANNEL, SLACK_WEB_HOOK, POSTCODE, HOUSENUMBER]), "Need to define all variables in environment variables"
...
def post_to_slack(text):
    ...
    response = requests.post(
        SLACK_WEB_HOOK, data=json.dumps(slack_data),
...
def query_once():
    ...
    available_slots = [slot for slot in slots if slot["bookedDeliveries"] < slot["maxDeliveries"]]
